...
function buildAssistantPrompt(message,sources=[],userContext=''){
...
async function askOpenAI(message,sources=[],history=[],reservationUsd=0,userContext=''){
  if(!process.env.OPENAI_API_KEY) throw new Error('OPENAI_API_KEY missing');
  const input=buildAssistantPrompt(message,sources,userContext);
