...
function redirectSuccess(res, result) {
  const ticket = jwt.sign({ purpose: 'social-login-result', token: result.token, userId: String(result.user._id), characterId: result.character.characterId }, secret(), { expiresIn: '2m' });
  const url = new URL('/onboarding', `${frontend()}/`); url.searchParams.set('social_login', 'verified'); url.searchParams.set('social_login_ticket', ticket); res.redirect(url.toString());
}
function redirectError(res, message) { const url = new URL('/onboarding', `${frontend()}/`); url.searchParams.set('social_login', 'error'); url.searchParams.set('social_login_message', String(message).slice(0, 180)); res.redirect(url.toString()); }

exports.start = (req, res) => {
  try {
    const provider = String(req.params.provider || '').toLowerCase();
    if (provider === 'google') {
      if (!process.env.GOOGLE_LOGIN_CLIENT_ID || !callback('google').startsWith('http')) throw new Error('Google Login non configurato');
    ...
    if (provider === 'github') {
      if (!process.env.GITHUB_OAUTH_CLIENT_ID || !callback('github').startsWith('http')) throw new Error('GitHub Login non configurato');
...
exports.callback = async (req, res) => {
  ...
  try {
    ...
    redirectSuccess(res, await upsertVerifiedAccount(provider, profile));
  } catch (error) { redirectError(res, error.message); }
};

exports.exchangeTicket = async (req, res) => {
  try { const data = jwt.verify(req.body?.ticket, secret()); if (data.purpose !== 'social-login-result') throw new Error(); res.json({ success: true, data: { token: data.token, userId: data.userId, characterId: data.characterId, metaverseVerified: true } }); }
