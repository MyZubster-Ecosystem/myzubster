...
function normalizeRewardComponents(rewardComponents, rewardAmount, currency = 'MYZ') {
  if (!rewardComponents || rewardComponents.length === 0) {
    return [{ asset: currency, amount: String(rewardAmount), status: 'ready' }];
  }
...
function hasPendingNonMyz(components) {
  return components.some(component => component.asset !== 'MYZ' && component.status !== 'paid');
}
  ...
  try {
    ...
    if (bounty) {
      ...
      bounty = new BountyConfig({
        ...
        repository,
        rewardAmount: amount,
        currency: components.length === 1 ? components[0].asset : 'MULTI',
    ...
    res.json({
      ...
      bounty: {
        ...
        rewardComponents: bounty.rewardComponents,
...
exports.getBounty = async (req, res) => {
  try {
    ...
    res.json({
      ...
      paymentWallet: bounty.paymentWallet,
  ...
  try {
    ...
    res.json({
      ...
      bounties: bounties.map(b => ({
        ...
        currency: b.currency,
        rewardComponents: b.rewardComponents,
        status: b.status,
        claimedBy: b.claimedBy,
        paymentWallet: b.paymentWallet,
        prNumber: b.prNumber
      }))
    });
  } catch (error) {
...
// Contributor wallet capture. Wallets are explicit per selected asset and become immutable once submitted.
exports.setPaymentWallets = async (req, res) => {
  try {
    const bounty = await BountyConfig.findOne({ issueNumber: req.params.issueNumber });
    if (!bounty) return res.status(404).json({ error: 'Bounty not found' });
    if (['paid', 'cancelled'].includes(bounty.status)) {
      return res.status(409).json({ error: 'Bounty is no longer accepting payment details' });
    }
    if (!bounty.rewardComponents?.length) {
      return res.status(409).json({ error: 'Bounty has no explicit reward components' });
    }
    ...
    for (const component of bounty.rewardComponents) {
      ...
      if (!wallet || typeof wallet !== 'string' || !wallet.trim()) {
        return res.status(400).json({ error: `Wallet required for ${component.asset}` });
      }
      if (component.walletAddress && component.walletAddress !== wallet.trim()) {
        return res.status(409).json({ error: `${component.asset} wallet is immutable; use cancellation/reissue` });
      }
...
exports.processMerge = async (req, res) => {
  try {
    ...
    for (const issueNumber of issueNumbers) {
      ...
      const components = bounty.rewardComponents?.length
        ? bounty.rewardComponents
        : normalizeRewardComponents(null, bounty.rewardAmount, bounty.currency);

      // Non-MYZ rails are deliberately pending until their Treasury/payment/verifier path is online.
      if (hasPendingNonMyz(components)) {
        ...
        processed.push({
          ...
          message: 'Non-MYZ reward rail is not yet online; no payment was submitted'
        });
        continue;
      }

      // Legacy MYZ path is preserved for existing bounties. New multi-asset payment rails must use
      // independent verification before transitioning to paid.
      try {
...
exports.getStats = async (req, res) => {
  try {
    ...
    res.json({
      ...
      paymentPending: pendingBounties,
      paid: paidBounties,
