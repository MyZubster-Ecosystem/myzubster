...
function configured() {
  return Boolean(process.env.FLYTEK_TELEGRAM_BOT_TOKEN);
}

function telegramApi(method) {
  const token = process.env.FLYTEK_TELEGRAM_BOT_TOKEN;
  if (!token) throw new Error('FLYTEK_TELEGRAM_BOT_TOKEN non configurato');
  ...
  switch (command) {
    ...
    case '/organize':
      return [
        ...
        '2. Verifica disponibilita e autorizzazione dello spazio.',
        '3. Controlla permessi, capienza, rumore, sicurezza e requisiti locali.',
        '4. Conferma sound system, artisti e timetable.',
        '5. Assegna crew per tecnica, welfare, hospitality, comunicazione e pulizia.',
        '6. Pubblica solo informazioni approvate dagli organizzatori.',
        '7. Prepara welfare, accessibilita, acqua, rifiuti e piano di emergenza.',
        '8. Archivia flyer, lineup e contributi con consenso.',
        '',
...
router.get('/status', (_req, res) => {
  res.json({
    ok: true,
    service: 'flytek-telegram-community-bot',
    configured: configured(),
    webhookSecretConfigured: Boolean(process.env.FLYTEK_TELEGRAM_WEBHOOK_SECRET),
    setupSecretConfigured: Boolean(process.env.FLYTEK_TELEGRAM_SETUP_SECRET)
  });
});
...
router.post('/webhook', async (req, res) => {
  ...
  // Ack quickly enough for Telegram while processing only supported text commands.
...
router.post('/setup', async (req, res) => {
  ...
  if (!setupSecret || req.get('x-flytek-setup-secret') !== setupSecret) {
    return res.status(401).json({ ok: false, error: 'Setup non autorizzato' });
  }
