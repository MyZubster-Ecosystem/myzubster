...
function createZorgaxDigitalBusinessRouter({
  ...
  router.post('/projects', authenticateMiddleware, async (req, res) => {
    try {
      const project = await service.createProject({
        ...
        customerProblem: req.body?.customerProblem,
  ...
  router.put('/projects/:projectId/strategy', authenticateMiddleware, async (req, res) => {
    try {
      const project = await service.updateStrategy({
        ProjectModel,
        ownerId: String(req.userId),
  ...
  router.post('/projects/:projectId/advance', authenticateMiddleware, async (req, res) => {
    try {
      const project = await service.advanceProject({
        ProjectModel,
        ownerId: String(req.userId),
        projectId: req.params.projectId,
