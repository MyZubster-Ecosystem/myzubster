...
router.get('/overview-ui', (req, res) => {
  res.type('html').send(`<!doctype html><html lang="it"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MyZubster · Utenti e Seller</title><style>:root{color-scheme:dark;font-family:system-ui,sans-serif}body{margin:0;background:#050b16;color:#f7f9fc}.wrap{max-width:760px;margin:auto;padding:28px 18px}h1{font-size:2rem;margin-bottom:8px}.sub{color:#aebbd0}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:24px 0}.card{background:#0d1828;border:1px solid #263a55;border-radius:20px;padding:22px}.label{color:#aebbd0;font-size:.9rem}.value{font-size:2.5rem;font-weight:900;margin-top:8px}.status{background:#111f32;border-radius:12px;padding:14px;color:#b8c3d4}button{border:0;border-radius:13px;padding:13px 18px;font-weight:800;color:#fff;background:linear-gradient(90deg,#7147ee,#078bd1)}@media(max-width:600px){.grid{grid-template-columns:1fr}.value{font-size:2.2rem}}</style></head><body><main class="wrap"><h1>Utenti e Seller</h1><div class="sub">Conteggi reali dal database MyZubster.</div><div class="grid"><div class="card"><div class="label">Iscritti totali</div><div class="value" id="users">—</div></div><div class="card"><div class="label">Seller totali</div><div class="value" id="sellers">—</div></div><div class="card"><div class="label">Seller attivi</div><div class="value" id="active">—</div></div></div><button id="refresh">Aggiorna</button><div id="status" class="status" style="margin-top:16px">Caricamento…</div></main><script>const s=document.getElementById('status'),b=document.getElementById('refresh');function token(){return localStorage.getItem('myzubster-token')||localStorage.getItem('token')||localStorage.getItem('accessToken')||sessionStorage.getItem('token')||''}async function load(){const t=token();if(!t){s.textContent='Sessione MyZubster non trovata. Accedi prima con il tuo account admin.';return}b.disabled=true;s.textContent='Aggiornamento…';try{const r=await fetch('/api/admin/dashboard/overview',{headers:{Authorization:'Bearer '+t}});const d=await r.json();if(!r.ok)throw new Error(d.error||('HTTP '+r.status));document.getElementById('users').textContent=d.totalUsers??'—';document.getElementById('sellers').textContent=d.totalSellers??'—';document.getElementById('active').textContent=d.activeSellers??'—';s.textContent='Dati aggiornati: '+new Date().toLocaleString('it-IT')}catch(e){s.textContent='Impossibile leggere i dati: '+e.message}finally{b.disabled=false}}b.onclick=load;load();</script></body></html>`);
});
