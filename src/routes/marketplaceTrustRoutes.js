...
router.patch('/orders/:id/status', authenticate, async (req, res) => {
  try {
    ...
    const allowed = {
      REQUESTED: seller ? ['ACCEPTED', 'CANCELLED'] : ['CANCELLED'],
      ACCEPTED: buyer || seller ? ['COMPLETED', 'CANCELLED'] : [],
      COMPLETED: [],
      CANCELLED: []
    };
    if (!allowed[order.status].includes(next)) return res.status(400).json({ success: false, message: 'Transizione di stato non valida' });
    order.status = next;
    await order.save();
...
router.post('/reports', authenticate, async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, reportId: report._id, status: report.status });
  } catch (error) {
