...
function requireModerator(req, res, next) {
  if (!['admin', 'moderator'].includes(req.userRole)) {
    return res.status(403).json({ success: false, message: 'Permessi moderazione insufficienti' });
  }
  next();
}
...
router.get('/orders/mine', authenticate, async (req, res) => {
  try {
    const orders = await MarketplaceOrder.find({ $or: [{ buyerId: req.userId }, { sellerId: req.userId }] })
      .populate('listingId', 'title status stock ownerUsername')
  ...
  try {
    ...
    if (order.status === 'REQUESTED' && next === 'ACCEPTED') {
      const listing = await MarketplaceListing.findOneAndUpdate(
        { _id: order.listingId, status: 'active', stock: { $gte: order.quantity } },
    ...
    if (order.status === 'ACCEPTED' && next === 'CANCELLED') {
      await MarketplaceListing.findByIdAndUpdate(order.listingId, { $inc: { stock: order.quantity }, $set: { status: 'active' } });
    }
...
router.get('/reputation/:userId', async (req, res) => {
  try {
    ...
    const [summary] = await MarketplaceReview.aggregate([
      { $match: { subjectId: new mongoose.Types.ObjectId(userId) } },
...
router.get('/moderation/reports', authenticate, requireModerator, async (req, res) => {
  try {
    ...
    const query = ['OPEN','REVIEWED','RESOLVED','DISMISSED'].includes(status) ? { status } : {};
    const reports = await MarketplaceReport.find(query)
      .populate('listingId', 'title status ownerUsername category')
      .sort({ createdAt: -1 }).limit(300).lean();
    res.json({ success: true, reports });
  } catch (_error) {
    res.status(500).json({ success: false, message: 'Impossibile recuperare le segnalazioni' });
  }
});

router.patch('/moderation/reports/:id', authenticate, requireModerator, async (req, res) => {
  try {
    const status = String(req.body?.status || 'REVIEWED').toUpperCase();
    const listingAction = String(req.body?.listingAction || 'none').toLowerCase();
    ...
    const report = await MarketplaceReport.findById(req.params.id);
    if (!report) return res.status(404).json({ success: false, message: 'Segnalazione non trovata' });
    if (listingAction !== 'none') {
      const mapped = { pause: 'paused', close: 'closed', activate: 'active' }[listingAction];
      await MarketplaceListing.findByIdAndUpdate(report.listingId, { $set: { status: mapped } });
    }
    report.status = status;
    report.reviewedBy = req.userId;
    report.reviewNote = String(req.body?.reviewNote || '').trim();
    report.reviewedAt = new Date();
    report.listingAction = listingAction;
    await report.save();
    res.json({ success: true, report });
  } catch (_error) {
    res.status(400).json({ success: false, message: 'Impossibile aggiornare la moderazione' });
  }
});
