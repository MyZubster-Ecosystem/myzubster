...
function verifyStripeSignature(rawBody, signatureHeader) {
  ...
  const expected = crypto.createHmac('sha256', process.env.STRIPE_WEBHOOK_SECRET).update(`${timestamp}.${rawBody.toString('utf8')}`, 'utf8').digest('hex');
...
async function syncStripeSubscription(subscription, eventId, fallbackUserId) {
  ...
  const now = new Date();
  const update = { paymentProvider:'STRIPE', stripeSubscriptionId:subscription.id, stripeCustomerId:typeof subscription.customer === 'string' ? subscription.customer : subscription.customer?.id, stripePriceId:subscription.items?.data?.[0]?.price?.id || process.env.STRIPE_SELLER_PRICE_ID, stripeSubscriptionStatus:subscription.status, stripeLastEventId:eventId, status, expiresAt:stripePeriodEnd(subscription) };
  if (status === 'ACTIVE') { update.verifiedAt=now; update.startsAt=subscription.start_date ? new Date(subscription.start_date*1000) : now; update.paymentReference=subscription.id; }
  ...
  try {
    ...
    if (membership.stripeCustomerId) {
      try {
        ...
        if (customerError.statusCode === 404 || /no such customer/i.test(customerError.message)) {
          console.warn('Seller checkout: clearing stale Stripe customer');
          membership.stripeCustomerId=undefined;
...
router.post('/cancel', authenticate, async (req,res) => {
  try {
    const membership=await SellerMembership.findOne({userId:req.userId}); if(!membership) return res.status(404).json({success:false,message:'Account Seller non trovato'});
    const now=new Date();
    if(membership.paymentProvider==='STRIPE' && membership.stripeSubscriptionId && process.env.STRIPE_SECRET_KEY) { const subscription=await stripeRequest('POST',`/v1/subscriptions/${encodeURIComponent(membership.stripeSubscriptionId)}`,{cancel_at_period_end:'true'}); membership.cancelledAt=now; membership.stripeSubscriptionStatus=subscription.status; membership.expiresAt=stripePeriodEnd(subscription)||membership.expiresAt; await membership.save(); return res.json({success:true,membership,cancellation:'at_period_end'}); }
    membership.status='CANCELLED'; membership.cancelledAt=now; await membership.save(); res.json({success:true,membership,cancellation:'immediate'});
  } catch(_error) { res.status(400).json({success:false,message:'Impossibile annullare il piano Seller'}); }
});

router.get('/moderation/pending', authenticate, requireModerator, async (_req,res) => { const memberships=await SellerMembership.find({status:'PENDING_PAYMENT'}).sort({createdAt:1}).limit(200).lean(); res.json({success:true,memberships}); });
router.patch('/moderation/:userId/activate', authenticate, requireModerator, async (req,res) => {
  try {
    const paymentReference=String(req.body?.paymentReference||'').trim(); if(!paymentReference||paymentReference.length<4||paymentReference.length>300) return res.status(400).json({success:false,message:'Riferimento pagamento verificato obbligatorio'});
