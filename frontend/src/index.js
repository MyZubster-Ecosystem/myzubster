...
marketplaceDemoVisualStyle.textContent = `
  section:has(> #demo-sellers-title)::before {
    ...
    background:
      #f6f9f6
      url('${MARKETPLACE_DEMO_IMAGE_BASE}/marketplace-demo-v2.png')
  ...
  .myz-marketplace-demo-card-visual {
    margin-top: 4px;
    border: 1px solid rgba(47, 158, 102, .25);
  }
...
function findMarketplaceCategorySelect() {
  return [...document.querySelectorAll('select')].find(select => {
    ...
    return (
      values.includes('event_equipment') &&
      values.includes('pet_services')
    );
  });
}
...
function mountMarketplaceDemoCategoryOverview() {
  ...
  if (
    !section ||
    section.querySelector('.myz-marketplace-demo-category-overview')
  ) {
    return;
  }
  ...
  overview.setAttribute(
    'aria-label',
    'Marketplace demo category explorer'
  );
  ...
  image.src =
    `${MARKETPLACE_DEMO_IMAGE_BASE}/marketplace-demo-categories.png`;

  image.alt =
    'MyZubster Marketplace Demo category overview';
  ...
  cardVisual.className =
    'myz-marketplace-demo-category-visual myz-marketplace-demo-card-visual';

  cardVisual.src =
    MARKETPLACE_CATEGORY_CARD_VISUAL;

  cardVisual.alt =
    'MyZubster Marketplace categories with demo card visuals';

  cardVisual.loading = 'lazy';
  cardVisual.decoding = 'async';
  ...
  nav.setAttribute(
    'aria-label',
    'Filter Marketplace Demo by category'
  );
  ...
  MARKETPLACE_DEMO_CATEGORIES.forEach(([value, label]) => {
    ...
    button.addEventListener(
      'click',
      () => setMarketplaceDemoCategory(value)
    );
  ...
  const firstContentAfterTitle =
    title.nextSibling;

  if (firstContentAfterTitle) {
    section.insertBefore(
      overview,
      firstContentAfterTitle
    );
  } else {
...
const marketplaceDemoObserver =
  new MutationObserver(() => {
    mountMarketplaceDemoCategoryOverview();
  });

marketplaceDemoObserver.observe(
  document.body,
  {
    childList: true,
    subtree: true
