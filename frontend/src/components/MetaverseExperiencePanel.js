...
function storageAvailable() {
  try {
    const key = '__myz_metaverse_check__';
...
function SystemCheck() {
  ...
  return (
    <section className="metaverse-panel">
      <h3>Requisiti e controllo sistema</h3>
      <div className={`metaverse-readiness ${ready ? 'is-ready' : 'needs-attention'}`}>
        {ready ? 'Browser pronto per Neon Plaza' : 'Alcune funzionalità potrebbero essere limitate'}
      </div>
      ...
      <details className="metaverse-requirements">
        <summary>Specifiche minime consigliate</summary>
        <p>Browser moderno aggiornato, JavaScript attivo, almeno 2 GB di RAM, schermo da 320 px e connessione stabile da 2 Mbps.</p>
        <p>Non sono richiesti GPU dedicata, visore VR o wallet. Per un’esperienza migliore: 4 GB di RAM e connessione da 5 Mbps.</p>
      </details>
    </section>
  );
}

function MetaverseExperiencePanel({
  identityStatus,
  online,
  nearby,
  messages,
  ...
  return (
    <>
      <section className="metaverse-panel">
        <h3>Dashboard sessione</h3>
        <div className="metaverse-stat-grid">
          <div><strong>{online}</strong><small>online</small></div>
          <div><strong>{nearby}</strong><small>vicini</small></div>
          <div><strong>{myMessages}</strong><small>messaggi</small></div>
          <div><strong>{visitedLandmarks.length}</strong><small>zone visitate</small></div>
        </div>
        <div className="metaverse-progress" aria-label={`${visitedLandmarks.length} zone visitate`}>
          <span style={{ width: `${Math.min(100, visitedLandmarks.length * 20)}%` }} />
        </div>
        <div className="metaverse-achievements">
          {badges.map((badge) => (
            <span
              key={badge.id}
              className={badge.unlocked ? 'is-unlocked' : 'is-locked'}
              title={badge.unlocked ? 'Sbloccato in questa esperienza' : 'Non ancora sbloccato'}
            >
        ...
        <small className="metaverse-muted">
          Questi sono progressi di esperienza, non attestazioni professionali o ricompense finanziarie.
        </small>
      </section>
