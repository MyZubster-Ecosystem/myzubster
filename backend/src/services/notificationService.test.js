...
test("creates one durable notification before realtime emit", async () => {
  ...
  Notification.create.mockImplementation(async (value) => ({
    ...value,
  ...
  expect(result.realtimeEmitted).toBeFalsy();
  expect(realtimeMetricsSnapshot().counters).toMatchObject({
    notificationAttempts: 1,
    notificationsPersisted: 1,
    notificationRealtimeMisses: 1,
  });
});
...
test("mark read returns refreshed unread count and realtime event", async () => {
  ...
  expect(emitToUser).toHaveBeenCalledWith(
    "u2",
    "notification.read",
    expect.objectContaining({ id: "n1", unreadCount: 2 }),
  );
});
