...
describe('Nicola N4K48 metaverse configurator', () => {
  const scriptPath = path.join(__dirname, '../scripts/metaverse/configure-n4k48.js');
  const packagePath = path.join(__dirname, '../package.json');
  const source = fs.readFileSync(scriptPath, 'utf8');
  const packageJson = JSON.parse(fs.readFileSync(packagePath, 'utf8'));

  test('exposes the bounded operator command', () => {
    expect(packageJson.scripts['metaverse:configure:n4k48'])
      .toBe('node scripts/metaverse/configure-n4k48.js');
  });

  test('targets Nicola public GitHub identity and approved character profile', () => {
    expect(source).toContain("const VERIFIED_GITHUB_LOGIN = 'nicolaususnicola-lgtm'");
    expect(source).toContain("const CHARACTER_NAME = 'N4K48'");
