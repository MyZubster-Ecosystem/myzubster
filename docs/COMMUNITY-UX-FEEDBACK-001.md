...
The current implementation is proposed in branch `zorgax/clarity-first-home-002`, rebuilt from the current `main` during the completion sprint:
...
- links kept on routes already present on `main`, so this UX change does not depend on the still-unmerged Chronicle Universe workstream.
